...
export async function updateTables(
  dynamoDbClient: DynamoDB,
  tables: UpdateTableInput[],
): Promise<void> {
  for (const table of tables) {
    await waitTillTableStateIsActive(dynamoDbClient, table.TableName)
    if (table.GlobalSecondaryIndexUpdates?.length) {
      const updateType = table.GlobalSecondaryIndexUpdates[0].Delete
        ? 'Deleting'
        : 'Creating'
      const indexName =
        updateType == 'Deleting'
...
export async function describeTables(
  dynamoDbClient: DynamoDB,
  tableNames: string[],
): Promise<Record<string, TableDescription>> {
...
export function getUpdateTableInput(
  createInput: CreateTableInput,
  existingTableConfig: TableDescription,
): UpdateTableInput[] {
  if (createInput.TableName !== existingTableConfig.TableName) {
    throw new Error('Invalid input, table name mismatch')
  }
  if (!createInput.GlobalSecondaryIndexes) {
    createInput.GlobalSecondaryIndexes = []
  }
  if (!existingTableConfig.GlobalSecondaryIndexes) {
    existingTableConfig.GlobalSecondaryIndexes = []
  }

  const inputGSINames = createInput.GlobalSecondaryIndexes.map(
    (index) => index.IndexName,
  )
  const existingGSINames = existingTableConfig.GlobalSecondaryIndexes.map(
    (index) => index.IndexName,
  )

  const indexNamesToAdd = inputGSINames.filter(
    (indexName) => !existingGSINames.includes(indexName),
  )
  const indexNamesToRemove = existingGSINames.filter(
    (indexName) => !indexName || !inputGSINames.includes(indexName),
  )

  const indicesToAdd: GlobalSecondaryIndexUpdate[] = indexNamesToAdd.map(
    (indexName) => {
      const idx = createInput.GlobalSecondaryIndexes?.find(
  ...
  const indicesToRemove: GlobalSecondaryIndexUpdate[] = indexNamesToRemove.map(
    (indexName) => {
      return {
        Delete: {
          IndexName: indexName,
